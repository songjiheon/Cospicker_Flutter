...
class _StaySearchScreenState extends State<StaySearchScreen> {
  ...
  Future<void> saveTourItemsToFirestore(
    ...
    await batch.commit();
    debugPrint("🔥 Firestore 저장 완료 (${items.length}개)");
  }
  ...
  Future<List<dynamic>> fetchTourApiLocationBased({
    ...
    while (accumulated.length < minItems) {
      ...
      debugPrint("📡 TourAPI 요청 (Page $pageNo, ContentType: $contentTypeId): $url");
      ...
      try {
        ...
        debugPrint("네트워크/파싱 오류 발생: $e");
        break;
      }
    }

    debugPrint("✅ 최종 누적 항목 수: ${accumulated.length}");
